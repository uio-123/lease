...
DESIGNED_FUNCTION_MODULES = [
    ("FM-01", "登录认证模块", "租客/管理员", "验证码、账号登录、JWT 令牌、当前用户信息", "已实现"),
    ("FM-02", "区域数据模块", "租客/运营管理员", "省市区列表、区域树和按区域筛选房源", "已实现"),
    ("FM-03", "公寓管理模块", "运营管理员", "公寓新增、修改、分页查询、上下架和详情维护", "已实现"),
    ("FM-04", "房间管理模块", "运营管理员", "房间新增、修改、分页查询、上下架和基础信息维护", "已实现"),
    ("FM-05", "房源搜索模块", "租客", "按区域、价格、标签、属性等条件查询公寓和房间", "已实现"),
    ("FM-06", "房间详情模块", "租客", "展示房间图片、设施、租金、支付方式和租期", "已实现"),
    ("FM-07", "预约看房模块", "租客/运营管理员", "租客提交预约，后台查询并更新预约状态", "已实现"),
    ("FM-08", "租约管理模块", "租客/运营管理员", "租约创建、查询、状态更新和详情查看", "已实现"),
    ("FM-09", "浏览历史模块", "租客", "记录租客浏览过的公寓与房间，支持分页查看", "已实现"),
    ("FM-10", "用户管理模块", "运营管理员/系统管理员", "租客用户、后台用户和账号状态维护", "已实现"),
    ("FM-11", "岗位管理模块", "系统管理员", "后台岗位维护和人员岗位归属管理", "已实现"),
    ("FM-12", "角色菜单权限模块", "系统管理员", "角色、菜单、用户角色关系和权限控制", "已实现"),
    ("FM-13", "属性管理模块", "运营管理员", "户型、朝向、面积、配套属性等字典维护", "部分实现"),
    ("FM-14", "费用/支付/租期配置模块", "运营管理员", "杂费类型、支付方式、租期配置和房间关联", "部分实现"),
    ("FM-15", "图片上传与对象存储模块", "运营管理员", "房源图片、头像、MinIO 上传和 URL 管理", "部分实现"),
    ("FM-16", "消息通知模块", "租客/运营管理员", "预约确认、租约状态和到期提醒通知", "设计扩展"),
    ("FM-17", "数据统计分析模块", "运营管理员", "房源数量、空置率、预约量和租约状态统计", "设计扩展"),
    ("FM-18", "系统配置与审计日志模块", "系统管理员", "系统配置、操作日志、异常日志和审计追踪", "设计扩展"),
]
...
def build_report() -> None:
    ...
    p(doc, "根据调整后的课程要求，本系统不再为了凑固定数量而机械划分模块，而是按实际接口、页面、数据表和业务闭环梳理功能模块。需求和设计阶段列出实际功能模块清单，编码实现阶段按代码完成情况说明已实现、部分实现和设计扩展范围。")
    ...
    add_table(
        doc,
        ["编号", "功能模块", "用户角色", "主要功能", "实现状态"],
        DESIGNED_FUNCTION_MODULES,
        widths=[0.75, 1.45, 1.3, 3.1, 1.0],
    )
    ...
    h(doc, "4.2.1 系统功能模块设计清单", 3)
    p(doc, "系统设计阶段按实际业务能力梳理出 18 个功能模块。模块设计覆盖前台租客业务、后台运营管理、系统管理与后续扩展能力；其中 12 个模块已实现，3 个模块部分实现，3 个模块作为后续扩展方向。")
    add_table(
        doc,
        ["编号", "模块名称", "面向角色", "设计说明", "实现状态"],
        DESIGNED_FUNCTION_MODULES,
        widths=[0.75, 1.45, 1.3, 3.1, 1.0],
    )
    h(doc, "4.3 软件系统结构", 2)
    ...
    h(doc, "5.4 编码实现范围说明", 2)
    p(doc, "根据实际代码、接口、页面和数据表支撑情况，本项目将功能模块分为已实现、部分实现和设计扩展三类。已实现模块覆盖系统主要业务闭环：用户登录、区域筛选、房源搜索、查看详情、预约看房、租约管理、后台公寓/房间维护、用户岗位与角色菜单权限管理；部分实现模块具备基础接口或数据支撑，但可在后续迭代中继续完善页面细节、业务规则和异常处理；设计扩展模块保留在报告中作为产品后续演进方向。")
    add_table(
        doc,
        ["范围", "模块数量", "模块"],
        [
            ("已实现", "12 个", "登录认证、区域数据、公寓管理、房间管理、房源搜索、房间详情、预约看房、租约管理、浏览历史、用户管理、岗位管理、角色菜单权限"),
            ("部分实现", "3 个", "属性管理、费用/支付/租期配置、图片上传与对象存储"),
            ("设计扩展", "3 个", "消息通知、数据统计分析、系统配置与审计日志"),
        ],
        widths=[1.3, 1.2, 4.6],
    )
    ...
    add_table(doc, ["要求", "报告位置", "完成说明"], [
        ...
        ("实际功能模块设计", "第 3.5 节、第 4.2.1 节", "按真实接口、页面、数据表和业务闭环列出功能模块、面向角色、设计说明和实现状态。"),
        ("编码实现范围说明", "第 5.4 节", "按实际完成情况说明已实现、部分实现和设计扩展模块。"),
        ("编码实现", "第 5 章", "说明后端、前端、本地部署、实现范围和实现效果截图位置。"),
